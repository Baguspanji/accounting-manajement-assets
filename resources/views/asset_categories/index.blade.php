...
@section('content')
    <div class="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
        <div>
            <h2 class="text-xl font-bold text-slate-800">Daftar Kategori Aset</h2>
            <p class="text-slate-500 text-sm">Kelola kategori aset tetap (Tanah, Bangunan, Kendaraan, dll).</p>
        </div>
            ...
            <table class="w-full text-left">
                <thead class="bg-slate-50 border-b border-slate-100 sticky top-0">
                    <tr>
                        ...
                        <th class="py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Nama Kategori</th>
                        <th class="py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Akun Aset</th>
                        <th class="py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Umur (th)</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($categories as $category)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="py-3 px-4 text-sm font-medium text-slate-800">{{ $category->name }}</td>
                            ...
                            <td class="py-3 px-4 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('asset-categories.show', $category->id) }}" class="p-1.5 hover:bg-info-light rounded-lg text-slate-500 hover:text-info">
                                        <i data-lucide="eye" class="w-4 h-4"></i>
                                    </a>
                                    <a href="{{ route('asset-categories.edit', $category->id) }}" class="p-1.5 hover:bg-warning-light rounded-lg text-slate-500 hover:text-warning">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-8 px-4 text-center text-slate-500">Belum ada data kategori aset.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
